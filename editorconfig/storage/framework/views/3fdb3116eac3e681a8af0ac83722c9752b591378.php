...
<button onclick="history.back()" type="button" class="inline-flex items-center gap-1.5 text-sm font-semibold text-primary-600 hover:text-primary-500 transition-all duration-150 hover:underline leading-6" title="Back">
  ...
  <form action="<?php echo e(route('add_update_client')); ?>" method="POST" enctype="multipart/form-data" class="space-y-8">
    ...
    <div class="flex flex-col gap-6 md:gap-x-12 xl:gap-16 md:items-start md:flex-row">
      ...
      <div class="<?php echo e(strpos($row['type'], '2') === false? 'hidden':''); ?> input-group" id="gst_selection">
        <label for="" class="block text-sm  after:content-['*'] after:ml-0.5 after:text-error-500 font-medium leading-6 text-gray-900">
          Select GST type 
        </label>
        ...
        <div class="flex items-center mt-2 gap-x-6 gap-y-4">
          <div class="relative flex items-start">
            <div class="flex items-center h-6">
              <input type="radio" value="1" <?php echo e($row['gst_type']== 1 ? 'checked':''); ?> name="gst_type" class="form-options radio" id="monthly">
            </div>
            <div class="ml-3 text-sm leading-6">
          ...
          <div class="relative flex items-start">
            <div class="flex items-center h-6">
              <input type="radio" value="2" <?php echo e($row['gst_type']== 2 ? 'checked':''); ?> id="quarterly" name="gst_type" class="form-options radio">
            </div>
            <div class="ml-3 text-sm leading-6">
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('accountant.layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u239499167/domains/digitalpahal.com/public_html/resources/views/accountant/clients/edit.blade.php ENDPATH**/ ?>