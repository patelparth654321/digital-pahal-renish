...
<section class="flex-1 py-6 overflow-y-auto">
  <div class="min-h-full px-4 sm:px-6">
    <a href="{{ isset($_GET['parentid']) ? route('view_client', ['id'=>$_GET['parentid']]) :  route('clients') }}" type="button" class="inline-flex items-center gap-1.5 text-sm font-semibold text-primary-600 hover:text-primary-500 transition-all duration-150 hover:underline leading-6" title="Back">
      ...
      Back to {{ (isset($_GET['parentid']) ? "Member": "Client") }}
    </a>
    <!-- START NAME & ACTIONS -->
    <div class="flex flex-col gap-4 xl:gap-6 xl:justify-between xl:flex-row xl:items-center mt-3">
      ...
      <div class="flex flex-col gap-4 sm:justify-start sm:flex-row sm:items-center xl:justify-end">
        <a href="{{ route('accoutant_data_summary', ['id'=>$row['client_id'], 'parentid'=>(isset($_GET['parentid'])? $_GET['parentid'] : '')]) }}" title="View Data Summary" class="btn btn-primary" role="button">
          View Data Summary
        </a>
        ...
        <div class="relative flex dropdown">
          ...
          <div class="absolute right-0 z-30 invisible w-56 mt-1 transition-all duration-100 origin-top-right scale-95 bg-white rounded-lg shadow-lg opacity-0 pointer-events-none top-full ring-1 ring-gray-200 focus:outline-none dropdown-menu" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
            <div class="divide-y divide-gray-100">
                ...
                <div class="flex items-start gap-2 px-4 py-2">
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-gray-900">
                      {{ $row['status'] ==1? "Disable Client":"Enable Client" }}
                    </p>
                  </div>

                  <div class="relative">
                    <input type="checkbox" {{ $row['status'] ==1?'checked':'' }} class="opacity-0 sr-only peer changeStatus" id="showFullPrompt" data-id="{{ $row['client_id'] }}" data-type="clients">
              ...
              <div class="py-1" role="none">
                <a href="{{ route('edit_client', ['id'=>$row['client_id']]) }}" title="" class="flex items-center gap-3 px-4 py-2 text-sm font-medium text-gray-700 transition-all duration-150 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900 whitespace-nowrap" role="menuitem" tabindex="-1">
                  <svg aria-hidden="true" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                ...
                <a href="javascript:void(0)" data-id="{{ $row['client_id'] }}" data-type="clients" class="flex items-center gap-3 px-4 py-2 text-sm font-medium transition-all duration-150 text-error-600 hover:bg-error-50 hover:text-error-700 focus:outline-none focus:bg-error-50 focus:text-error-700 whitespace-nowrap deleteData" data-toggle="modal" data-target="#deleteConfirmationModal">
                  <svg aria-hidden="true" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                    <path d="M4 7h16"></path>
              ...
              <div class="grid grid-cols-1 mt-4 sm:grid-cols-2 xl:grid-cols-3 gap-x-4 gap-y-3">
                ...
                <div class="flex items-start gap-6 p-3 text-gray-600 rounded-md bg-gray-50 ring-1 ring-inset ring-gray-500/10">
                  ...
                  <div class="flex items-center justify-end gap-2">
                    ...
                    @endif
                    <a href="javascript:void(0)" data-id="{{ $ud['id'] }}" data-type="clients/document" class="relative flex p-1 transition-all duration-150 rounded hover:ring-gray-300 ring-1 ring-transparent {{ $ud['view_status'] =='0'? 'documentChangeStatus': 'text-success-600'}}" aria-label="{{ $ud['view_status'] =='0'? 'Mark as Viewed': 'Already Viewed'}}" data-microtip-position="top" role="tooltip">
                    ...
                    <button type="button" class="relative flex p-1 transition-all duration-150 rounded text-error-600 hover:text-error-700 hover:bg-gray-200 hover:ring-gray-300 ring-1 ring-transparent deleteData" data-toggle="modal" data-target="#deleteConfirmationModal" data-id="{{ $ud['id'] }}" data-type="clients/documents" data-refresh="true">
                      <span class="sr-only">
                        Delete
                      </span>
                      <svg aria-hidden="true" class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
    ...
    @if($row['added_by'] == auth()->user()->id && count($members) > 0)
    <ul class="flex items-center w-full mt-6 space-x-4 overflow-x-auto border-b border-gray-200 tabs" id="myTab" role="tablist">
      <li class="tab-item" role="presentation">
                ...
                <tbody class="bg-white divide-y divide-gray-200 list">
                  ...
                  <tr>
                    ...
                    <td class="py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6 whitespace-nowrap">
                      <a href="{{ route('view_client_member', ['id'=>$c['client_id'], 'parentid'=>$row['client_id']])}}">
                    ...
                    <td class="relative py-4 pl-3 pr-4 whitespace-nowrap sm:pr-6">
                      <div class="flex items-center gap-4">
                        ...
                        <a href="{{ route('view_client_member', ['id'=>$c['client_id'], 'parentid'=>$row['client_id']])}}" title="" class="btn btn-icon btn-secondary-light btn-icon-transparent" aria-label="View Member" data-microtip-position="top" role="tooltip">
                          <span class="sr-only">
                            View Member
                          </span>
                          <svg aria-hidden="true" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
  ...
  <script>
    $(document).on('click', '.tab_click', function(e) {
      ...
      const nextState = {};
      const nextURL = "{{ route('view_client', ['id'=>$row['client_id']])}}" + "?type=" + type;
      window.history.pushState(nextState, nextTitle, nextURL);
    })
    $(document).on('change', '#year', function(e) {
      let url = "{{ route('view_client', ['id'=>$row['client_id']])}}?type=" + $('#selected_tab').val();
