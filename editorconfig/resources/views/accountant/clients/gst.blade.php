...
<section class="flex-1 py-6 overflow-y-auto">
    <div class="min-h-full px-4 sm:px-6">
        <a href="{{ route('accoutant_data_summary', ['id'=>$data['id'], 'parentid'=>(isset($_GET['parentid'])? $_GET['parentid'] : '')]) }}" class="inline-flex items-center gap-1.5 text-sm font-semibold text-primary-600 hover:text-primary-500 transition-all duration-150 hover:underline leading-6" title="Back">
                    ...
                    <div>
                        ...
                        <div>
                            <select name="" id="month_search" class="form-select">
                                ...
                                <option {{ $isDisabled ? "disabled":"" }} {{ (isset($_GET['month']) && $_GET['month']==$key) ? "selected" : ( !(isset($_GET['month'])) && $key == date('m') ? "selected":"")  }} value="{{$key}}">
                                    {{$value}}
                                </option>
                    ...
                    <div class="px-4 py-6">
                        <div class="flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
                            <div class="flex items-center flex-1 gap-4">
                                <img class="object-cover w-12 h-12 bg-gray-300 rounded-full shrink-0" src="{{  $d['icon'] ? (asset('images/document-logos/'.$d['icon'])) : asset('images/icon-document.svg')}}" alt="">
                                <div class="flex-1 py-0.5 min-w-0">
                                    <p class="text-base font-semibold text-gray-600">
                                        {{$d['title']}}
                                    </p>
                                    <p class="text-sm text-gray-600">Amount: {{env('CURRENCY').$d['count'] }}</p>
                                </div>
                            </div>
                                ...
                                <div class="absolute right-0 z-30 invisible w-40 mt-1 transition-all duration-100 origin-top-right scale-95 bg-white rounded-lg shadow-lg opacity-0 pointer-events-none top-full ring-1 ring-gray-200 focus:outline-none dropdown-menu" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                    <div class="divide-y divide-gray-100">
                                        <div class="py-1" role="none">
                                            @foreach($d['sub_types'] as $st)
                                            <a href="javascript:void(0)" class="flex items-center px-4 py-2 text-sm font-medium text-gray-700 transition-all duration-150 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900 whitespace-nowrap upload_file" role="menuitem" tabindex="-1" data-toggle="modal" data-target="#fileUploadModal" sub-type="{{ $st['key'] }}" data-type="{{ $d['key'] }}" data-name="{{ $d['title'] }}">
                                                {{ $st['title'] }}
                                            </a>
                                            @endforeach
                                        </div>
            ...
            <div class="relative flex flex-col w-full max-h-full overflow-hidden bg-white border border-gray-200 shadow-xl outline-none pointer-events-auto rounded-xl sm:m-4" role="document">
                ...
                <div class="flex-1 px-4 pb-4 overflow-y-auto sm:px-6 sm:pb-6 grow">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                        <div id="start_month_selection" class="sm:col-span-2">
...
<input type="hidden" value="{{ $_GET['client_id'] }}" id="client_id" />
...
<script>
    ...
    var config = {
        route: "{{ route('accountant_dashboard') }}"
    };
</script>
<script src="{{ asset('js/gst-config.js')}}"></script>
<!-- File item template -->
...
<script>
    ...
    $(document).on('change', '#month_search', function(e) {
        let url = "{{ route('accoutant_gst')}}?client_id={{$_GET['client_id']}}&parentid={{isset($_GET['parentid']) ? $_GET['parentid'] : '' }}";
