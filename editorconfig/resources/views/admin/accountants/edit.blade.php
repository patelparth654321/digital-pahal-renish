...
<div class="min-h-full px-4 sm:px-6">
  ...
  <form action="{{ route('add_update_accountant') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
    @csrf
    <input type="hidden" value="{{$row->accountant_id}}" name="user_id" />
    ...
    <div class="flex flex-col gap-6 md:gap-x-12 xl:gap-16 md:items-start md:flex-row">
      ...
      <div class="grid flex-1 max-w-3xl grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
        ...
        <div class="input-group">
          ...
          <div class="mt-1.5 input-wrapper">
            <input type="tel" name="alternate_number" value="{{ $row['alternate_number'] }}" class="form-input">
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-6 md:gap-x-12 xl:gap-16 md:items-start md:flex-row">
      ...
      <div class="grid flex-1 max-w-3xl grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
          ...
          <div>
            ...
            <div class="relative flex items-center gap-5 mt-2">
              <div class="flex-shrink-0 w-12 h-12 overflow-hidden rounded-lg ring-1 ring-gray-200">
                <img class="object-cover w-full h-full image_file" src="{{ getImagePath($row->logo, 'accountant/logo') }}" alt="">
              </div>
          ...
          <div class="flex items-center mt-2 gap-x-6 gap-y-4">
            <div class="relative flex items-start">
              <div class="flex items-center h-6">
                <input type="radio" value="0" {{$row['is_gst']==0 ? 'checked' : ''}} name="is_gst" class="form-options radio">
              </div>
            ...
            <div class="relative flex items-start">
              <div class="flex items-center h-6">
                <input type="radio" value="1" {{$row['is_gst']==1 ? 'checked' : ''}} id="withGst" name="is_gst" class="form-options radio">
              </div>
              <div class="ml-3 text-sm leading-6">
        ...
        <div class="input-group {{$row['is_gst']==0 ? 'hidden' : ''}}" id="withGstInput">
          <label for="" class="required">
            GST Number
          </label>
          <div class="mt-1.5 input-wrapper">
            <input type="text" name="gst_number" id="gst_number" value="{{ $row['gst_number'] }}" class="form-input">
          </div>
        </div>
        ...
        <div class="input-group sm:col-span-2">
          ...
          <div class="mt-1.5 flex items-center gap-3">
            <div class="flex-shrink-0 w-12 h-12 overflow-hidden rounded-lg ring-1 ring-gray-200">
              <img class="object-cover w-full h-full image_file" src="{{ getImagePath($row->adhaar_card, 'accountant/aadhaar') }}" alt="">
            </div>
            <div class="flex-1 file-input">
        ...
        <div class="input-group sm:col-span-2">
          ...
          <div class="mt-1.5 flex items-center gap-3">
            <div class="flex-shrink-0 w-12 h-12 overflow-hidden rounded-lg ring-1 ring-gray-200">
              <img class="object-cover w-full h-full image_file" src="{{ getImagePath($row->pan_card, 'accountant/pan') }}" alt="">
            </div>
            <div class="flex-1 file-input">
    ...
    <div class="flex flex-col gap-6 md:gap-x-12 xl:gap-16 md:items-start md:flex-row">
      ...
      <div class="grid flex-1 max-w-3xl grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
        <div class="input-group">
          ...
          <div class="mt-1.5 input-wrapper">
            <input type="email" name="support_email_id" value="{{ $row['support_email_id'] }}" class="form-input" required>
          </div>
        </div>
        ...
        <div class="input-group">
          ...
          <div class="mt-1.5 input-wrapper">
            <input type="tel" name="support_phone" value="{{ $row['support_phone'] }}" class="form-input" required>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-6 md:gap-x-12 xl:gap-16 md:items-start md:flex-row">
      ...
      <div class="">
        ...
        <div class="grid grid-cols-1 mt-2 sm:grid-cols-2 gap-x-6 gap-y-4">
          <div class="relative flex items-start">
            <div class="flex items-center h-6">
              <input type="checkbox" id="gst" aria-describedby="gst-description" name="type[]" value="2" {{ strpos($row['type'], '2') !== false? 'checked':'' }} class="form-options checkbox">
            </div>
            <div class="ml-3 text-sm leading-6">
    ...
    <div class="flex flex-col gap-6 md:gap-x-12 xl:gap-16 md:items-start md:flex-row">
        ...
        <div class="input-group">
          ...
          <div class="mt-1.5 input-wrapper">
            <select name="plan_id" id="plan_id" class="form-input" required>
              ...
              @foreach($plans as $p)
              <option {{ $p['id'] ==$row['plan_id'] ? "selected":"" }} value="{{ $p['id'] }}">{{ $p['name'] }}</option>
              @endforeach
            </select>
          </div>
        </div>
      ...
      <!-- <div class="grid flex-1 max-w-3xl grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
        <div class="input-group">
          ...
          <div class="mt-1.5 input-wrapper">
            <input type="number" name="discount" max="100" min="0" value="{{ $row['discount'] }}" class="form-input">
          </div>
        </div>
      </div> -->
    </div>
...
<script>
  ...
  $(document).on('click', '.remove', function() {
    ...
    if(data=='logo'){
      $(this).parent().find('.image_file').attr('src', "{{ getImagePath($row->logo, 'accountant/logo') }}");
    }else if(data=='aadhaar'){
      $(this).parent().find('.image_file').attr('src', "{{ getImagePath($row->adhaar_card, 'accountant/aadhaar') }}");
    }else {
      $(this).parent().find('.image_file').attr('src', "{{ getImagePath($row->pan_card, 'accountant/pan') }}");
    } 
