  ...
  <div class="px-4 sm:px-6">
    <ul class="flex items-center w-full space-x-4 overflow-x-auto tabs" id="myTab" role="tablist">
      ...
      <li class="tab-item" role="presentation">
        <button type="button" id="{{$row['key']}}-tab" class="tab_click tab-link {{ (($key === array_key_first($rows) && !isset($_GET['type'])) || (isset($_GET['type']) && $_GET['type'] == $row['key']))? 'active': '' }}" data-toggle="tab" data-target="#{{$row['key']}}" role="tab" aria-controls="{{$row['key']}}" aria-selected="true">
          {{$row['title']}}
        </button>
      </li>
      @endforeach
...
<section class="flex-1 py-6 overflow-y-auto">
  <div class="min-h-full px-4 sm:px-6">
    <div class="pb-6 tab-content" id="myTabContent">
      @foreach($rows as $key => $row)
      <div class="hidden transition-opacity duration-150 ease-linear bg-transparent opacity-0 tab-pane {{ (($key === array_key_first($rows) && !isset($_GET['type'])) || (isset($_GET['type']) && $_GET['type'] == $row['key']))? 'show': '' }}" id="{{$row['key']}}" role="tabpanel" aria-labelledby="{{$row['key']}}-tab">
...
<input id="client_id" value="{{ auth()->user()->id }}" type="hidden" />
...
<script>
  ...
  $(document).on('click', '.tab_click', function(e) {
    ...
    const nextState = {};
    const nextURL = "{{ route('dms') }}" + "?type=" + type;
    window.history.pushState(nextState, nextTitle, nextURL);
  })

  $(document).on('change', '#month_search', function(e) {
    let url = "{{ route('dms')}}?type=" + $('#selected_tab').val();
