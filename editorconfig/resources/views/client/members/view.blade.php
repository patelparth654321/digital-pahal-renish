...
<section class="flex-1 py-6 overflow-y-auto">
  <div class="min-h-full px-4 sm:px-6">
    <a href="{{ route('members') }}" class="inline-flex items-center gap-1.5 text-sm font-semibold text-primary-600 hover:text-primary-500 transition-all duration-150 hover:underline leading-6" title="Back">
    ...
    <div class="flex flex-col gap-4 xl:gap-6 xl:justify-between xl:flex-row xl:items-center mt-3">
      ...
      <div class="flex flex-col gap-4 sm:justify-start sm:flex-row sm:items-center xl:justify-end">
        ...
        <a href="{{ route('gst', ['client_id'=>$data['client_id'] ]) }}" title="View GST Details" class="w-full btn btn-primary sm:w-auto" role="button">
          View GST
        </a>
        @endif
        <div class="relative flex dropdown">
          ...
          <div class="absolute right-0 z-30 invisible w-56 mt-1 transition-all duration-100 origin-top-right scale-95 bg-white rounded-lg shadow-lg opacity-0 pointer-events-none top-full ring-1 ring-gray-200 focus:outline-none dropdown-menu" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
            <div class="divide-y divide-gray-100">
              ...
              <a href="{{ route('member_details', ['id'=>$data['client_id']]) }}" title="" class="flex items-center gap-3 px-4 py-2 text-sm font-medium text-gray-700 transition-all duration-150 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900 whitespace-nowrap" role="menuitem" tabindex="-1">
                  <svg aria-hidden="true" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
              ...
              @if($data['client_id']!= auth()->user()->id)
                <a href="{{ route('edit_member', ['id'=>$data['client_id']]) }}" title="" class="flex items-center gap-3 px-4 py-2 text-sm font-medium text-gray-700 transition-all duration-150 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900 whitespace-nowrap" role="menuitem" tabindex="-1">
                  <svg aria-hidden="true" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                    <path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0"></path>
                    <path d="M6 21v-2a4 4 0 0 1 4 -4h3.5"></path>
                ...
                <a href="javascript:void(0)" data-id="{{ $data['client_id'] }}" data-type="members" class="flex items-center gap-3 px-4 py-2 text-sm font-medium transition-all duration-150 text-error-600 hover:bg-error-50 hover:text-error-700 focus:outline-none focus:bg-error-50 focus:text-error-700 whitespace-nowrap deleteData" data-toggle="modal" data-target="#deleteConfirmationModal">
                  <svg aria-hidden="true" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                    <path d="M4 7h16"></path>
    ...
    <div class="pb-6 tab-content mt-6" id="myTabContent">

      @foreach($docs as $key => $row)
      <div class="hidden transition-opacity duration-150 ease-linear bg-transparent opacity-0 tab-pane {{ (($key === array_key_first($docs) && !isset($_GET['type'])) || (isset($_GET['type']) && $_GET['type'] == $row['key']))? 'show': '' }}" id="{{$row['key']}}" role="tabpanel" aria-labelledby="{{$row['key']}}-tab">
        <!-- START DOCUMENTS LIST -->
        @if($row['key'] =="gst_billing_documents")
        ...
        @endif
        <div class="space-y-4 {{ $row['key'] =='gst_billing_documents'? 'mt-4':'' }}">
          @foreach($row['subdocs'] as $d)
          <div class="transition-all duration-150 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md">
            <div class="px-4 py-6">
              <div class="flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
                ...
                @if(($row['key'] =="personal_documents" && count($d['uploaded_docs']) == 0) || $row['key'] !="personal_documents")
                <!--  -->
                <div class="flex items-center justify-end gap-4">
                  <button type="button" class="btn btn-sm btn-secondary upload_document_button" data-toggle="modal" data-target="#uploadSellDocumentsModal" parent-data-type="{{ $row['key'] }}" data-type="{{ $d['key'] }}" data-name="{{ $d['title'] }}">
              ...
              <div class="grid grid-cols-1 mt-4 sm:grid-cols-2 xl:grid-cols-3 gap-x-4 gap-y-3">
                ...
                <div class="flex items-start gap-6 p-3 text-gray-600 rounded-md bg-gray-50 ring-1 ring-inset ring-gray-500/10">
                  ...
                  <div class="flex items-center justify-end gap-2">
                    @if($ud['category'] && $ud['category']=="With Pass")
                    <span class="badge badge-white">
                      {{ $ud['category'].' '.(":".$ud['password']) }}
                    </span>
                    @endif
                    @if($ud['month'])
                    <span class="badge badge-white">
                    ...
                    <a href="javascript:void(0)" class="relative flex p-1 transition-all duration-150 rounded hover:ring-gray-300 ring-1 ring-transparent {{ $ud['view_status'] =='1'? 'text-success-600': ''}}" aria-label="{{ $ud['view_status'] =='0'? 'Not viewed yet': 'Viewed by Admin'}}" data-microtip-position="top" role="tooltip">
                      {!! getViewStatus($ud['view_status']) !!}
                    </a>
                    ...
                    <button type="button" class="relative flex p-1 transition-all duration-150 rounded text-error-600 hover:text-error-700 hover:bg-gray-200 hover:ring-gray-300 ring-1 ring-transparent deleteData" aria-label="Delete" data-toggle="modal" data-target="#deleteConfirmationModal" data-id="{{ $ud['id'] }}" data-type="dms" data-refresh="true">
                      <span class="sr-only">
                        Delete
                      </span>
                      <svg aria-hidden="true" class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
              <div id="month_selection">
                ...
                <div class="mt-1.5 input-wrapper">
                  <select name="month" id="month" class="pl-3 pr-10 form-select">
                    ...
                    @php $isDisabled = false; $currentMonth=date('m'); @endphp
                    @foreach (getMonths() as $key=>$value)
                    @if($key == $currentMonth + 01) @php $isDisabled=true; @endphp @endif
                    <option {{ $isDisabled ? "disabled":"" }} value="{{$key}}">
                      {{$value}}
                    </option>
              ...
              <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:col-span-2">
                ...
                <div id="type_selection">
                  <label for="" class="required">Enter Document Name</label>
                  <div class="mt-1.5 input-wrapper">
                    <input type="text" name="sub_type" id="sub_type" class="pl-3 pr-10 form-input" />
                  ...
                  <div class="flex items-center mt-2 gap-x-6 gap-y-4">
                    <div class="relative flex items-start">
                      <div class="flex items-center h-6">
                        <input type="radio" id="withPass" value="With Pass" name="category" class="form-options radio">
                      </div>
                    ...
                    <div class="relative flex items-start">
                      <div class="flex items-center h-6">
                        <input type="radio" value="Without Pass" id="withoutPass" name="category" class="form-options radio" checked>
                      </div>
                      <div class="ml-3 text-sm leading-6">
  ...
  <input id="client_id" value="{{ $data['client_id'] }}" type="hidden" />
  ...
  <script>
    ...
    $(document).on('click', '.upload_document_button', function(e) {
      ...
      if($(this).attr('data-type') == "personal_other_documents" || $(this).attr('data-type') == "tax_other_documents"){
        $('#sub_type').attr('required', true);
        $('#type_selection').show();
      }
    });
    ...
    $(document).on('click', '.tab_click', function(e) {
      ...
      const nextState = {};
      const nextURL = "{{ route('view_member', ['id'=>$data['client_id']])}}" + "?type=" + type;
      window.history.pushState(nextState, nextTitle, nextURL);
    })

    $(document).on('change', '#month_search', function(e) {
      let url = "{{ route('view_member', ['id'=>$data['client_id']])}}?type=" + $('#selected_tab').val();
